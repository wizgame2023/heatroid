...
namespace basecross {
	...
	void GameStage::CreatePlayer() {
		...
		ptrPlayer->GetComponent<Transform>()->SetPosition(Vec3(75, 5.0125f, 0));
	...
	void GameStage::CreateGimmick()
	{
		...
		for (auto& v : LineVec) {
			...
			//各値がそろったのでオブジェクト作成
			auto door = AddGameObject<GimmickDoor>(Pos, Rot, Scale, 1.0f, 1.0f, Switch);
		}
		...
		for (auto& v : LineVec) {
			...
			AddGameObject<GimmickButton>(Pos, Rot, Scale, 1.0f, 1.0f, Button);
		}
